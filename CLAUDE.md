...
cd scripts && ./status_docker_services.sh

# Start all services manually
cd scripts && ./startup_docker_services.sh

# Test ansible syntax
cd ansible && ansible-playbook servyy.yml --syntax-check

# Regenerate service .env files
cd ansible && ansible-playbook servyy.yml --tags "user.docker.env"
...
- You are currently in: `/home/cda/dev/infrastructure/container` (development machine)
- All commands above run from this local repository
- Deployment targets: `lehel.xyz` (production), `servyy-test.lxd` (test container)
...
1. [Overview](#overview)
2. [Critical: git-crypt Workflow](#critical-git-crypt-workflow)
3. [Architecture](#architecture)
4. [Service Naming Convention](#service-naming-convention)
5. [Services](#services)
...
13. [Documenting Changes](#documenting-changes)
...
## Critical: git-crypt Workflow

**⚠️ IMPORTANT:** Most files in this repository are encrypted with git-crypt. You MUST understand this workflow before making changes.

### What is Encrypted

Per `.gitattributes`, these patterns are encrypted:
- All `docker-compose.yml` files
- All `*.yaml` files
- All `*.env` files
- All `*.conf` files
- All files matching `secrets.*` or `secret_*`
- `git/htpasswd`

### Working with Encrypted Files

**CRITICAL RULE:**
- **Encrypted files appear as plaintext** when the repository is unlocked
- You can read and edit them normally in the local development environment
- They are automatically encrypted when committed to git
- On servers, Ansible automatically unlocks the repository during deployment
...
# If not unlocked, you'll need the encryption key
# (This is typically already set up on development machines)
```

**When Adding New Files:**
- If a file matches patterns in `.gitattributes`, it's automatically encrypted
- To encrypt a new pattern, add it to `.gitattributes`:
  ```bash
  echo "new-pattern/** filter=git-crypt diff=git-crypt" >> .gitattributes
  ```

**Important Notes:**
- Never commit unencrypted secrets to git
- All `docker-compose.yml` files contain credentials and are encrypted
- The encryption is transparent once unlocked - just edit files normally
- Check `git-crypt status` to verify which files are encrypted

### Context: Development Machine vs. Deployment Server

**Development Machine (where you are now):**
- Repository is unlocked with git-crypt
- Files appear as plaintext
- You can freely edit docker-compose.yml, .env files, etc.
- Changes are automatically encrypted on commit

**Deployment Server (production/test):**
- Ansible unlocks git-crypt during deployment
- Services run from `/home/user/containers/{service}/`
- .env files are generated by Ansible (not committed to git)
- docker-compose.yml files are unlocked from git
...
```
ansible/
├── servyy.yml              # Main playbook (imports all others)
├── servyy.sh               # Wrapper script: ansible-playbook servyy.yml -i production
├── servyy-test.sh          # Test environment deployment
├── inventory/
│   ├── production          # Production hosts (lehel.xyz, aqui.fritz.box)
│   └── testing_inventory   # Test container inventory
└── plays/
    ├── system.yml          # System-level setup (packages, user, storage)
    ├── user.yml            # Docker + services deployment
    ├── testing.yml         # Test container setup
    ├── leaguespehere.yml   # LeagueSphere-specific deployment
    ├── roles/
    │   ├── system/         # System role (packages, monit, swap)
    │   ├── user/           # User role (Docker, services, backups)
    │   ├── testing/        # Testing role (LXD container setup)
    │   ├── ls_setup/       # LeagueSphere setup
    │   ├── ls_app/         # LeagueSphere application
    │   └── ls_access/      # LeagueSphere access control
    └── vars/
...
# With extra args (e.g., tags, limits)
./servyy.sh --tags "docker" --limit lehel.xyz
...
# Regenerate .env files for all services
ansible-playbook servyy.yml --tags "user.docker.env"
```

### LeagueSphere Deployment

LeagueSphere is a separate application with its own Ansible roles:

**Roles:**
- `ls_setup/` - Infrastructure setup
- `ls_app/` - Application deployment
- `ls_access/` - Access control and permissions

**Playbook:**
```bash
# Deployed via plays/leaguespehere.yml
# Imported automatically by servyy.yml
```

This is deployed alongside the main servyy-container services but has its own configuration and lifecycle.
...
### History Directory Convention

The `history/` directory contains logs of significant infrastructure changes. This is a **required practice** for tracking major modifications.

**When to Create a History Log:**
- Removing services or major components
- Significant refactoring affecting multiple files
- Breaking changes to configuration
- Security-related updates
- Migration from one technology to another (e.g., DuckDNS → Porkbun)

**Naming Convention:**
```
history/YYYY-MM-DD_<change_description>.md
```
...
touch history/2025-11-03_service_removal.md

# Document what was removed, why, and how to rollback
# Include original file contents for reference
# Update history/README.md to list the new log
```

**See:** `history/2025-11-03_duckdns_removal.md` for a complete example.
...
Examples:
  ...
  chore: update traefik configuration
  fix: photoprism indexing script timeout
  docs: update CLAUDE.md with git-crypt workflow
```

### Branch Naming

- `master` - Production-ready code (main branch for PRs)
- `claude/*` - Claude Code development branches
- Feature branches as needed
