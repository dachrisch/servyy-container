...
| claude-hub | claude-hub.hub | *(none — `traefik.enable=false`, SSH/CLI only)* | Headless Claude Code session dispatcher on `codey.lehel.xyz`; the hub session is still a foreground `tmux`-wrapped process (`docker exec -it claude-hub.hub tmux attach -t hub`), but a task session it dispatches is now a native `claude --bg --remote-control` job in its own git-worktree workspace (`spinup-repo.sh` → vendored `spinup-session.mjs`), lifecycle-managed by a vendored `session-reaper.mjs` (park/revive/close-on-request, every 15min) plus a weekly force-cleanup backstop for long-parked sessions — see `claude-hub/vendor/VENDORED.md` and `history/2026-09-26_claude-hub-worktree-dispatch-rework.md`. No public URL. Shares its `/root/dev` checkouts with `opencode` via the `claude_shared_dev_checkouts` named volume (opencode must deploy first for the `external: true` reference to resolve — see `history/2026-09-22_claude-hub-service.md`) |
